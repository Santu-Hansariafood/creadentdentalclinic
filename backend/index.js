...
const startServer = async () => {
  ...
  const storageRoutesLogger = (prefix) => (req, res, next) => {
    console.log(
      "[STORAGE-MOUNT:%s] method=%s path=%s originalUrl=%s ct=%s cl=%s",
      prefix,
      req.method,
      req.path,
      req.originalUrl,
  ...
  if (process.env.ICICI_STALE_CLEANUP_DISABLED !== "true") {
    try {
      ...
      setTimeout(async () => {
        console.log("Running initial stale transaction cleanup...");
        try {
          await cleanupStaleTransactions(24);
        } catch (e) {
          console.error("Initial stale cleanup failed:", e.message);
        }
        setInterval(async () => {
          console.log("Running periodic stale transaction cleanup...");
          try {
            await cleanupStaleTransactions(24);
          } catch (e) {
            console.error("Periodic stale cleanup failed:", e.message);
          }
        }, interval);
      }, initialDelay);
      console.log(
        "Scheduled stale transaction cleanup: first run in 60s, then every 6h (removes >24h old INITIATED/REQ/PENDING).",
      );
    } catch (schedErr) {
      console.error(
        "Failed to schedule stale transaction cleanup:",
        schedErr.message,
      );
    }
  } else {
    console.log(
      "Stale transaction cleanup scheduler is disabled via ICICI_STALE_CLEANUP_DISABLED=true.",
    );
  }
  ...
  const publicSeo = {
    "/": {
      ...
      description:
        "Looking for the best dental clinic in Salt Lake, Kolkata? Creadent Dental Clinic offers expert dentists, root canal treatment, dental implants, braces, teeth whitening, and family dental care.",
    },
    "/about-us": {
      title: "About Us | Best Dental Clinic in Salt Lake Kolkata",
      description:
        "Learn about Creadent Dental Clinic in Salt Lake, Kolkata, our patient-first approach, and our commitment to modern, dependable dental care.",
    },
    "/contact-us": {
      title: "Contact Us | Creadent Dental Clinic Salt Lake Kolkata",
      description:
        "Contact Creadent Dental Clinic in Salt Lake, Kolkata for appointments, payment support, and patient enquiries.",
    },
    "/google-reviews": {
      title: "Google Reviews | Creadent Dental Clinic Salt Lake Kolkata",
      description:
        "Read Google reviews for Creadent Dental Clinic in Salt Lake, Kolkata, and share your experience with our dental team.",
    },
    "/privacy-policy": {
      title: "Privacy Policy | Creadent Dental Clinic Kolkata",
      description:
        "Read the privacy policy for Creadent Dental Clinic to understand how we securely handle patient, visitor, and payment information.",
    },
    "/terms-and-conditions": {
      title: "Terms and Conditions | Creadent Dental Clinic",
      description:
        "Review the terms and conditions for using the Creadent Dental Clinic website, online booking, and payment processing services.",
    },
    "/refund-policy": {
      title: "Refund Policy | Creadent Dental Clinic",
      description:
        "Read the refund policy for Creadent Dental Clinic to understand how online payment refunds and transaction reversals are handled.",
    },
    "/cancellation-policy": {
  ...
  app.get("/sitemap.xml", async (req, res) => {
    ...
    const staticPages = Object.keys(publicSeo).map((pagePath) => ({
      ...
      priority:
        pagePath === "/"
          ? "1.0"
          : pagePath === "/about-us" || pagePath === "/contact-us"
            ? "0.9"
            : "0.7",
      changefreq:
        pagePath === "/" || pagePath === "/google-reviews"
          ? "weekly"
          : "monthly",
    }));
...
${staticPages
  ...
  app.use((req, res) => {
    console.log(
      "[404-ROUTE-NOT-FOUND] method=%s path=%s originalUrl=%s contentType=%s",
      req.method,
      req.path,
      req.originalUrl,
      req.headers["content-type"],
