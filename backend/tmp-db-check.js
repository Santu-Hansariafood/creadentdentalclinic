...
(async () => {
  try {
    ...
    console.log(badUsers.map(u => ({ email: u.email, phone: u.phone, role: u.role })));

    const patients = await Patient.find().lean();
    console.log('patients', patients.length);
    console.log(patients.map(p => ({ email: p.email, phone: p.phone, userId: p.userId })));
    ...
    console.log(badPatients.map(p => ({ email: p.email, phone: p.phone, userId: p.userId })));
    ...
    for (const t of tests) {
      ...
      const userExists = await User.findOne({ $and: [{ $or: orConditions }, { role: t.role }] });
      console.log('graphQL register test', t, 'query', JSON.stringify({ $and: [{ $or: orConditions }, { role: t.role }] }), 'userExists', userExists ? { email: userExists.email, phone: userExists.phone, role: userExists.role } : null);
    }
